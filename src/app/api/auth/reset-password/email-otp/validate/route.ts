...
export async function POST(request: NextRequest) {
  try {
    ...
    const url = `${manualAuthConfig.ssoServerUrl}/public/reset-password/email-otp/validate`;
    ...
    const response = await fetch(
      `${manualAuthConfig.ssoServerUrl}/public/reset-password/email-otp/validate`,
      {
        method: "POST",
        headers,
    ...
    if (!parsed.ok) {
      return NextResponse.json(
        { error: "email_otp_validation_failed", message: parsed.text },
    ...
    console.error("Email OTP validation error:", (error as Error).message);
    return NextResponse.json(
