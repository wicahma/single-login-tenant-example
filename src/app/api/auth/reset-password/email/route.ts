...
export async function POST(request: NextRequest) {
  try {
    ...
    const url = `${manualAuthConfig.ssoBaseUrl}/api/public/reset-password/email`;
    ...
    const response = await fetch(
      `${manualAuthConfig.ssoServerUrl}/public/reset-password/email`,
      {
        method: "POST",
        headers,
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: "reset_email_failed", message: errorText },
    ...
    console.error("Reset password email error:", (error as Error).message);
    return NextResponse.json(
