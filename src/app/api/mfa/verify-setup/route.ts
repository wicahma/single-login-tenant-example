...
export async function POST(request: NextRequest) {
  try {
    ...
    const url = `${manualAuthConfig.mfaServerUrl}/api/v1/mfa/verify-setup`;
    ...
    const response = await fetch(
      `${manualAuthConfig.mfaServerUrl}/api/v1/mfa/verify-setup`,
      {
        method: "POST",
        headers,
    ...
    if (!parsed.ok) {
      return NextResponse.json(
        { error: "mfa_verify_setup_failed", message: parsed.text },
        { status: parsed.status },
      );
    }

    const data = parsed.data;
    console.log("MFA verify setup successful:", data);
    return NextResponse.json(data);
  } catch (error) {
    console.log("MFA verify setup error:", (error as Error).message);
    return NextResponse.json(
