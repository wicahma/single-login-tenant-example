...
export async function POST(request: NextRequest) {
  try {
    ...
    const url = `${manualAuthConfig.mfaServerUrl}/api/v1/mfa/verify-recovery`;
    ...
    const response = await fetch(
      `${manualAuthConfig.mfaServerUrl}/api/v1/mfa/verify-recovery`,
      {
        method: "POST",
        headers,
    ...
    console.log("MFA verify recovery error:", (error as Error).message);
    return NextResponse.json(
